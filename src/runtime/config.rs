...
impl RuntimeConfig {
    pub fn validate(&self) -> Result<(), String> {
        validate_runtime_file_path("state_file", &self.state_file)?;
        validate_runtime_file_path("log_file", &self.log_file)?;
...
fn validate_runtime_file_path(name: &str, path: &Path) -> Result<(), String> {
    if path.as_os_str().is_empty() {
        return Err(format!("runtime {name} must not be empty"));
    }

    if path.is_absolute() {
        return Err(format!("runtime {name} must be relative"));
    }
    ...
    {
        return Err(format!(
            "runtime {name} must stay within the runtime directory"
        ));
    }
...
mod tests {
    ...
    #[test]
    fn runtime_config_rejects_absolute_state_path() {
        let config = RuntimeConfig {
            state_file: std::env::temp_dir().join("runtime.state.json"),
            ..RuntimeConfig::default()
        };

        let err = config
            .validate()
            .expect_err("absolute state path should be rejected");

        assert!(err.contains("state_file"));
        assert!(err.contains("relative"));
    }

    #[test]
    fn runtime_config_rejects_parent_log_path() {
        let config = RuntimeConfig {
            log_file: PathBuf::from("../runtime.log"),
            ..RuntimeConfig::default()
        };
        ...
        let err = config
            .validate()
            .expect_err("parent log path should be rejected");

        assert!(err.contains("log_file"));
